...
class MigrationJob(Base):
    ...
    # ── Semantic-model reuse detection columns ──
    WorkspaceId = Column(String(100), nullable=True)
    DatasourceName = Column(String(255), nullable=True)
    DatasetId = Column(String(100), nullable=True)
...
def update_job_status(job_id: int, job_update: JobUpdate, db: Session = Depends(get_db)):
    ...
    # Only overwrite these when the caller actually sent them — a plain
    # "Running" or "Failed" update (which never includes these fields)
    # would otherwise NULL out a previously-set DatasourceName/DatasetId.
    if job_update.DatasourceName is not None:
        db_job.DatasourceName = job_update.DatasourceName
    if job_update.DatasetId is not None:
        db_job.DatasetId = job_update.DatasetId
